...
export const getUserPosts = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(id).populate("posts").populate("createdBy", "username phoneNumber profilepic");

        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }

        res.status(200).json({ message: "User posts retrieved", posts: user.posts });
    } catch (error) {
