...
export const removeCart = async (req,res)=>{
    ...
    const userId = req.user.id
    try {
        const existingPost = await Cart.FindOne({postId,userId})
        if(!existingPost){
            return res.status(400).json({message:"post does not exist"})
        }

        await User.findByIdAndUpdate(postId, { $pull: { cart: existingPost ._id}})
        await Cart.findByIdAndDelete(existingPost)
