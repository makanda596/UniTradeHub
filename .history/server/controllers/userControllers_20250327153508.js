...
export const userprofile = async(req,res)=>{
    const {id}= req.params
    try {
        const user = await User.findOne({ _id: id }).populate("posts", "productName description createdAt").populate("reviews", "text");
        if(!user){
...
export const usergetposts = async (req, res) => {
    const { id } = req.params
    try {
        const user = await User.findOne({ _id: id }).populate("posts", "productName description createdBy createdAt")
        if (!user) {
            res.status(400).json({ message: "user not found" })
        }
        res.status(200).json(user)
