...
//     const { id } = req.params;
//     const { username, email, phoneNumber, password, bio, location, profilepic } = req.body;
...
//         const responspic = await cloudinary.uploader.upload(profilepic)
...
//             updatedFields.profilepic = responspic.secure_url
//         }
...
export const updateUser = async (req, res) => {
    ...
    //    -  This example assumes you have user data in `req.user` from a middleware (e.g., JWT auth).
    if (req.user.id !== id && req.user.role !== 'admin') {
        return res.status(403).json({ message: "Unauthorized: You can only update your own profile." });
    }

    try {
        // 2. Input Validation (using express-validator, recommended)
        //    -  You can define validation rules in a separate middleware or directly in the route handler.
        //    -  This is a basic example; adjust the rules as needed.
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        }

        // 3. Sanitize Input (using express-validator)
        const sanitizedUsername = username ? sanitize(username).escape() : undefined;
        const sanitizedEmail = email ? sanitize(email).normalizeEmail() : undefined;
        const sanitizedBio = bio ? sanitize(bio).escape() : undefined;
        const sanitizedLocation = location ? sanitize(location).escape() : undefined;
        ...
        // 5. Check for duplicate username/email/phone (EXCLUDING the current user)
        ...
        // 6. Handle Profile Picture Upload
        let profilePicUrl = user.profilepic; // Keep the old URL by default
        if (profilepic) { // Only upload if a new picture is provided
            try {
                const responspic = await cloudinary.uploader.upload(profilepic);
                profilePicUrl = responspic.secure_url;
                if (user.profilepic) {
        ...
        // 9.  Remove password from the response (SECURITY)
        const userWithoutPassword = updatedUser.toObject(); // Convert Mongoose doc to plain object
