...
import { sendEmail,sendRestPasswordConfirmationEmail } from "../utilis/sendEmail.js";
import { VERIFICATION_EMAIL_TEMPLATE } from "../Email.js/Emailtemplates.js";
...
export  const signup = async (req,res)=>{
    try {
        ...
        const verificationCodeExpires = Date.now() + 60* 60 * 100; 
        ...
        await user.save();
        await VERIFICATION_EMAIL_TEMPLATE(email, verificationCode)
...
export const login = async (req, res) => {
    ...
    const generateToken = (id) => {
        return jwt.sign({id}, process.env.SECTRET_KEY, { expiresIn: "15mins " })
    }
    try {
...
export const userprofile = async(req,res)=>{
    const {id}= req.params
    try {
        const user = await User.findOne({ _id: id }).populate("posts", "productName description createdAt").populate("reviews", "text senderId");
        if(!user){
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        await sendEmail(email,  `http://localhost:5173/ResetPassword/${resetToken}`);
...
export const resetPassword = async (req, res) => {
  ...
  try {
    ...
    await user.save();
      sendRestPasswordConfirmationEmail(user.email)
