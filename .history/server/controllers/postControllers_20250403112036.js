...
export const getpost= async (req, res) => {
    try {
        ...
        if (!category) return res.status(400).json({ error: "Category is required" }).sort({createdAt:-1});
 
        const posts = await Post.find({ category }).populate("createdBy", "username phoneNumber profilepic");
...
export const getUserPosts = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findById(id).populate({
            path: "posts",
            select: "productName image description createdAt"
        }).sort({createdAt:-1});
