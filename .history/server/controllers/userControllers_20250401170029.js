...
export const EmailVerificationResend = async (req, res) => {
    ...
    try {
        // 1. Find user by email (corrected syntax)
        ...
        // 2. Check if user is already verified (uncommented and fixed)
        if (user.isVerified) {
            return res.status(400).json({ message: 'User already verified. Please login.' });
        }

        // 3. Generate new verification code and expiration
        const verificationCodeExpires = Date.now() + 15 * 60 * 1000; // Fixed to 15 minutes (was 1.5 minutes)
        const verificationCode = crypto.randomBytes(2).toString("hex").toUpperCase(); // Added uppercase for better readability
...
export const userprofile = async(req,res)=>{
    ...
    try {
        ...
        .populate("posts", "productName description createdAt")
