...
export const login = async (req,res) => {
    try {
        ...
        res.setHeader("Authorization", `Bearer ${generateAccessToken(user)}`);
      return  res.cookie("token", generateAccessToken(user), {
            httpOnly: true,
            secure: true,
            sameSite: "None",
        ...
        res.status(200).json({
            ...
            user: {
                ...
                gender: user.gender,
                profilepic: user.profilepic, // Include avatar (profile picture)
                createdAt: user.createdAt, // Optional: if your schema tracks it
            },
        });
        // req.session.user = ({
        //     _id:user._id,
