...
export const getcart = async (req,res)=>{
    try {
        const user = await Post.findB(req.user.i)
        if(!user){
...
export const removeCart = async (req,res)=>{
    ...
    const userId = req.user.id
    try {
        // i am checking if there is any existing post por not 
        const existingPost = await Cart.findOneAndDelete({postId:postId,userId:userId})
        if(!existingPost){
            return res.status(400).json({message:"post does not exist or already deleted"})
        }

        ///we find the userId in the user schema and pull the itme which is the cart.witht the existing id from the cart schema then i set it to true
