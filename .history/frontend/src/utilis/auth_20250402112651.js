...
export const useAuthStore = create((set,get) => ({
    ...
    login: async (email, password) => {
        set({ isLoading: true, error: null });
        try {
            const response = await axios.post('https://unitradehubback.onrender.com/auth/login', { email, password });
   ...
   SendAnotherCode :async (email) => {
       set({ isLoading: true, error: null });
        ...
        try {
            const response = await axios.post(`${USER_API}/auth/resendcode`, { email })
    ...
    forgotPassword: async (email) => {
        set({ isLoading: true, error: null });
         try {
            const response = await axios.post(`${USER_API}/auth/password`, { email });
