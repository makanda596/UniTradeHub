...
const AllPosts = () => {
    ...
    const defaultPostImage = "https://source.unsplash.com/600x400/?technology,gadgets"; // Default image for all posts
                ...
                <ul className="space-y-6">
                    {posts.length > 0 ? (
                        posts.map((post, index) => (
                            <li key={index} className="p-6 bg-gray-50 rounded-lg shadow-md hover:shadow-lg transition duration-300 flex flex-col sm:flex-row items-center gap-6">
                                {/* Post Image */}
                                <img src={defaultPostImage} alt="Post" className="w-48 h-32 object-cover rounded-lg" />
