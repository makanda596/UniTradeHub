...
const ReportPost = ({ popUp, setPopUp, postId, postName }) => {
    const { reportPost } = useAuthStore(); // Make sure you have reportPost action in your auth store
    ...
    const reportFunction = async () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await reportPost(postId); // Update to use postId instead of receiverId
                    setPopUp(false);
                    Swal.fire({
                        icon: 'success',
                        title: 'Post Reported!',
