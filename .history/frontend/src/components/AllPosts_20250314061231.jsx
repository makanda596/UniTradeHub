...
import { FaThumbsUp, FaComment, FaShare, FaWhatsapp } from "react-icons/fa"; // Import icons
...
const AllPosts = ({ user }) => {
        ...
        setPosts((prevPosts) => {
            ...
            setLikedPosts((prevLikedPosts) => {
                ...
                if (updatedLikedPosts[index]) {
                    ...
                    delete updatedLikedPosts[index];
                } else {
                    updatedPosts[index].likes += 1;
    ...
    // Function to handle comments (placeholder for now)
    const handleComment = () => {
                        ...
                        posts.map((post, index) => (
                            <li key={index} className="p-6 bg-gray-50 rounded-lg shadow-md hover:shadow-lg transition duration-300 flex flex-col gap-4">
                                ...
                                <div className="flex justify-between items-center mt-4 text-gray-600">
                                    <button onClick={() => handleLike(index)} className={`flex items-center gap-2 ${likedPosts[index] ? 'text-blue-600' : ''} transition`}>
                                        <FaThumbsUp /> Like ({post.likes || 0})
                                    </button>
                                    <button onClick={handleComment} className="flex items-center gap-2 hover:text-blue-600 transition">
                                        <FaComment /> Comment
                                    </button>
                                    <a href={`https://wa.me/?text=Check out this post: ${post.productName}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-green-600 transition">
                                        <FaWhatsapp /> WhatsApp
                                    </a>
                                    <button onClick={() => handleShare(post)} className="flex items-center gap-2 hover:text-blue-600 transition">
                                        <FaShare /> Share
                                    </button>
                                </div>
                            </li>
