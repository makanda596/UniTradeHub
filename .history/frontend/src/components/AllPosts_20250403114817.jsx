    ...
    return (
        <div className="flex flex-col flex-1 max-w-xl mx-auto  mt-0">
          ...
          {!isCreating ? (
            ...
            <div className="py-6 bg-red-600">
                <h1 className="text-xl font-bold text-gray-800 mb-2 ml-6"> All Posts</h1>
                <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-6">
                    {posts.length > 0 ? (
                        posts.map((post, index) => (

                            <li key={index} className="pl-4 pb-4 bg-black shadow-md hover:shadow-lg transition duration-300 flex flex-col gap-2">
                                ...
                                <Link to={`/Onepost/${post.category}/${post._id}`}>
                                    <div>
                                        ...
                                        <p className="text-gray-600">{post.description.length > 60 ? (<p>{post.description.slice(0, 60)}...<span className="text-blue-500">see more</span></p>) : (<>{post.description}</>)}</p>
                                    </div>

                                    <img src={post.image} alt="Post" className="w-full -ml-1 h-96 object-cover " />
