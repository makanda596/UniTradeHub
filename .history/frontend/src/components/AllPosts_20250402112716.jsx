...
const AllPosts = ({ user }) => {
    ...
    const POST_URL = import.meta.env.VITE_POST_API;
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                const response = await axios.get('https://unitradehubback.onrender.com/posts/allposts', {
                            ...
                            <li key={index} className="pl-4  bg-white shadow-md hover:shadow-lg transition duration-300 flex flex-col gap-2">
                                ...
                                <Link to={`/Onepost/${post.category}/${post._id}`}>
                                    <div>
                                        ...
                                        <p className="text-gray-600">{post.description.length > 60 ? (<p>{post.description.slice(0, 60)}...</p>) : (<>{post.description}</>)}</p>
                                    </div>
