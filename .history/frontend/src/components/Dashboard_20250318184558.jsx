...
const Dashboard = ({ user }) => {
    ...
    const countPosts = async ()=>{
        try{
        const response = await axios.get(`http://localhost:5000/auth/countposts/${user._id}`)
            ...
            <div className="pl-4 rounded-sm md:rounded-xl shadow-lg 
                ...
                <div className="mt-2 bg-blue-300 p-1 rounded-lg text-center">
                    ...
                    <ul className="text-gray-700 mt-2">
                        <a href={`/Myposts/${user?._id}` }className="py-1 flex text-blue-600 justify-between">Posts <span className="font-bold">{count}</span></a>
