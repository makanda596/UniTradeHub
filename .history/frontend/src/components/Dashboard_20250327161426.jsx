...
const Dashboard = ({ user, userId }) => {
    ...
    const countReview = async () => {
        try {
            ...
            setCountReviews(response.data.reviewCount);  // Store only the review count
        } catch (error) {
            setError(error.message);
            ...
            <div className="pl-4 rounded-sm bg-white md:rounded-xl shadow-lg 
                ...
                <div className="mt-2 bg-blue-300 p-1 rounded-lg text-center">
                    ...
                    <ul className="text-gray-700 mt-2">
                        <a href={`/Myposts/${user?._id}`} className="py-1 flex text-blue-600 justify-between">
                            Posts <span className="font-bold">{count.postCount}</span>
                        </a>
                        <li className="py-1 flex justify-between">
                            Reviews <span className="font-bold">{countReviews}</span>
                        </li>
                        <li className="py-1 flex justify-between">
                            Bookmarked <span className="font-bold">0</span>
                        </li>
                        <li className="py-1 flex justify-between">
                            Alerts <span className="font-bold">0</span>
                        </li>
                    </ul>
                </div>
            </div>
