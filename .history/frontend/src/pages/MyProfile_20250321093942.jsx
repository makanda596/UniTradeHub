  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-100">
      ...
      <main className="flex-1 p-6">
        ...
        <div className="text-center mb-6 md:hidden flex flex-col items-center">
          <img
            src="https://i.pravatar.cc/100"
            alt="User Avatar"
            className="w-24 h-24 rounded-full border-4 border-blue-500 shadow-md"
          />
          <h2 className="text-xl font-semibold mt-3">{user?.name || "Brian Makanda"}</h2>
          <p className="text-gray-500 text-sm">{user?.email || "oumab743@gmail.com"}</p>
        </div>

        {/* Navigation Links */}
        <ul className="space-y-4 text-gray-700 bg-white p-4 rounded-lg shadow-md">
          <a
            href={`/Myposts/${user?._id}`}
            className="flex items-center space-x-3 hover:bg-blue-50 p-3 rounded-md transition duration-200"
          >
            <FaShoppingCart className="text-blue-500" />
            <span className="font-medium">My Posts</span>
          </a>
          <li className="flex items-center space-x-3 hover:bg-green-50 p-3 rounded-md cursor-pointer transition duration-200">
            <FaInbox className="text-green-500" />
            <span className="font-medium">Inbox</span>
          </li>
          <li className="flex items-center space-x-3 hover:bg-red-50 p-3 rounded-md cursor-pointer transition duration-200">
            <FaHeart className="text-red-500" />
            <span className="font-medium">Wishlist</span>
          </li>
          <a
            href="/Settings"
            className="flex items-center space-x-3 hover:bg-gray-50 p-3 rounded-md transition duration-200"
          >
        ...
        {/* Account Overview */}
        <h2 className="text-2xl font-semibold mt-8 mb-6 text-center md:text-left">
          Account Overview
        </h2>
