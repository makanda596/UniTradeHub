...
const UserProfile = ({userId}) => {
  ...
  const { countfollowersfunction, countfollowingfunction, countReviwesFunction, CountReviews } = FollowStore()
   
    useEffect(()=>{
      const fetchAlldata = async ()=>{
        await countfollowersfunction()
        await countfollowingfunction()
        await countReviwesFunction(userId)
      };
      fetchAlldata()
    },[])
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try { 
        ...
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/auth/profile/${userId}`,{
  ...
  const handleAddToCart = async () => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/addcart/67dd2d8a2200fb944d0ed820`);
          ...
          <div className="w-full md:w-1/3 space-y-6">
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="pt-12 px-6 pb-2">
                ...
                <p className="mt-3 text-gray-600"><span className="font-bold tet-black">Bio:</span> {user?.bio || "No bio available"}</p>
                ...
                <p>reviews{CountReviews ||"mnone"}</p>
                <div className="mt-4 flex space-x-4">
                  <button
                    onClick={() => window.location.href = `/Chart/${user?._id}`}
                    className="flex items-center justify-center px-2 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition "
                  >
                  ...
                  <button
                    ...
                    className="flex items-center px-2 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition"
                  >
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex-shrink-0">
                              <img
                                src={user?.profilepic || "/default-profile.png"}
                                alt="User"
                                className="w-8 h-8 rounded-full shadow-sm border-2 border-blue-200"
                              />
                            </div>
                          </div>
