...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/user-profile"); // Replace with actual API
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="mt-6 bg-white p-6 rounded-lg shadow-lg">
          ...
          <div className="mt-4">
            {user?.posts?.length > 0 ? (
              user.posts.map((post) => (
                <div key={post.id} className="mt-4 p-4 border border-gray-200 rounded-lg shadow-md">
                  ...
                  {post.image && <img src={post.image} alt="Post" className="mt-3 rounded-lg" />}
                </div>
              ))
            ) : (
