          ...
          {posts.length > 0 ? (
            posts.map((saved, index) => (
              <div key={index} className="border p-4 rounded-lg shadow-sm">
                <p>seller {saved.userId?.username}</p>
                {saved.userId?.profilepic && <img src={saved.userId.profilepic} alt="" className="w-16 h-16 rounded-full object-cover mb-2" />}
                <p className="text-lg">name {saved.postId?.productName}</p>
                <p className="text-lg">description {saved.postId?.description}</p>
                {saved.postId?.image && <img src={saved.postId.image} alt="" className="w-full h-48 object-cover rounded-md mb-2" />}
                <button
