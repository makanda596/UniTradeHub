...
const FollowersFollowing = () => {
    ...
    const fetchFollowing = async () => {
        try {
            ...
            console.log(res.data.following.followedId)
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            {loading ? <p>Loading...</p> : (
                <div className="grid grid-cols-2 gap-6">
                        ...
                        {followers.length > 0 ? (
                            followers.map(user => (
                                <div key={user._id} className="flex items-center gap-3 p-2 border rounded-md mb-2">
                                    <img src={user?.followedId?.profilepic || "https://via.placeholder.com/50"} className="w-10 h-10 rounded-full" alt="Profile" />
                    ...
                    {/* Following Section */}
                    <div>
                        <h3 className="text-lg font-medium mb-2">Following klffjfjpo</h3>
                            {following.map((item,index)=>{
                                <div key={index}>
                                    <p className="text-red-600">kfv{item?.createdAt ||"no ro"}</p>
                                </div>
                            })}
                    </div>
                </div>
