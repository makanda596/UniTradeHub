...
const UserProfile = () => {
  ...
  const handleAddToCart = async () => {
    try {
      const response = await axios.post(`http://localhost:5000/auth/addcart/${user?.posts._id}`);
      console.log(user?.posts._id)
          ...
          {user?.posts?.length > 0 ? (
            user.posts.map((post) => (
              <div key={post._id} className="p-4 rounded-lg shadow-md bg-gray-100">
                ...
                <p className="mt-3 text-gray-700 font-semibold">{post._id}</p>
                ...
                {/* Add to Cart Button */}
                <button
                  onClick={() => handleAddToCart(post._id, post.productName)}
