...
const ProfilePage = ({ user, logout, userId }) => {
  ...
  const { usercountfollowersfunction,  usercountfollowingfunction } = FollowStore()
 
  useEffect(()=>{
    const fetchAlldata = async ()=>{
      await usercountfollowersfunction()
      await usercountfollowingfunction()
    };
    fetchAlldata()
  },[])
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch review count 
        const reviewsResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/reviews/countReviews`, { 
          headers: { Authorization: `Bearer ${token}` },
        });
        setCountReviews(reviewsResponse.data);

        // Fetch post count
        const postsResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/auth/countposts`,{
          headers: { Authorization: `Bearer ${token}` },
        });
