        ...
        try {
            ...
            const response = await axios.put(
                `http://localhost:5000/auth/update/${user._id}`,
                {
                    ...
                    email,
                    phoneNumber,
                    password: password || undefined  // Only send password if it's entered
                },
                    ...
                    <div className="w-full md:w-3/4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" className="w-full p-2 border border-gray-300 rounded" placeholder={user?.username} value={username} onChange={(e) => setUsername(e.target.value)} />
                            </div>

                            <div>
                                <label className="block text-gray-700">Email {user?.email}</label>
                                <input type="email" className="w-full p-2 border border-gray-300 rounded" placeholder={user?.email} value={email} onChange={(e) => setEmail(e.target.value)} />
                            </div>

                            <div>
                                <label className="block text-gray-700">Phone Number</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded" placeholder={user?.phoneNumber} value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} />
                            </div>
