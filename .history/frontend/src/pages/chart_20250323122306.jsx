...
export default function ChatCard({ user }) {
    ...
    const fetchMessages = async () => {
        try {
            ...
            const res = await axios.get(`http://localhost:5000/conversation/getconversation/${receiverId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });

            if (res.data?.conversation?.messages) {
                setConversation(res.data.conversation.messages); // Set messages array
            } else {
                setConversation([]); // Set empty if no messages
            }
            ...
            <div className="h-60 overflow-y-auto space-y-3 p-2">
                {conversation.length > 0 ? (
                    conversation.map((msg, index) => (
                        <div key={index} className={`flex ${msg.sender === user._id ? "items-end justify-end" : "items-start"}`}>
                            <div className={`p-3 rounded-lg max-w-xs ${msg.sender === user._id ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}>
                                {msg.content}
                            </div>
                        </div>
                    ))
